# RagSwift — Research Paper Assistant
...
RagSwift uses [uv](https://github.com/astral-sh/uv) for dependency management.

```bash
# Clone the repository
git clone <repo-url>
cd rag-RagSwift-project
...
| **Session isolation** | Each session gets its own Qdrant collection (`RagSwift_{session_id}`) and a separate LangGraph SQLite checkpointer thread — prevents cross-session data leakage |
...
RagSwift includes an automated RAG evaluation pipeline (`evaluate.py`) built on [DeepEval](https://github.com/confident-ai/deepeval).
