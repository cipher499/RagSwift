...
def relevancy_check_node(state: RAGState) -> dict:
    query = state["query"]                          # always exists; if it doesn't, the graph will throw a KeyError
    docs = state.get("retrieved_docs") or []        # doesn't always exist, if it does, fetch, else create an empty list

    doc_snippets = "\n\n---\n\n".join(doc.page_content[:300] for doc in docs[:3])   # takes first 300 chars of each chunk and joins them 
                                                                                    # reduces latency, token usage, and enough for LLM to judge relevance
    if not doc_snippets:                                            
